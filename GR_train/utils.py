...
from data import SeqRecDataset, RetrievalDataset
...
def parse_dataset_args(parser):
    parser.add_argument("--data_path", type=str, default="../data",
                        help="data directory")
    parser.add_argument("--tasks", type=str, default="retrieval",
...
def parse_train_args(parser):
    ...
    parser.add_argument("--qd_contrastive", type=float, default=1.0)
...
def parse_test_args(parser):
    ...
    parser.add_argument("--metrics", type=str, default="recall@1,recall@5,recall@10,recall@20,recall@50,recall@100,mrr@10",
                        help="test metrics, separate by comma")
    parser.add_argument("--test_task", type=str, default="retrieval")
...
def load_datasets(args):
    ...
    for task, prompt_sample_num,data_sample_num in zip(tasks,train_prompt_sample_num,train_data_sample_num):
        ...
        elif task.lower() == "retrieval":
            dataset = RetrievalDataset(args, mode="train", prompt_sample_num=prompt_sample_num, sample_num=data_sample_num)
        else:
    ...
    if task.lower() == "seqrec":
        ...
        valid_data = RetrievalDataset(args,"test",args.valid_prompt_sample_num)

    return train_data, valid_data

def load_test_dataset(args):

    print(args.test_task)
    if args.test_task.lower() == "seqrec":
        test_data = SeqRecDataset(args, mode="test", sample_num=args.sample_num)
    elif args.test_task.lower() == "retrieval":
        test_data = RetrievalDataset(args, mode="test", sample_num=args.sample_num)
    else:
        raise NotImplementedError
